...
public class UserManagerSvcImpl implements UserManagerSvc {
	...
	public void registerCheckNameInit(Request request, Response response) {
		// get option from main
	
	}
	/*
	public void registerFull(Request request, Response response) {
		try {
	...
	*/
	public void authenticate(Request request, Response response) {
		...
		utilSvc.validateParams(request, response);
		if ((Boolean) request.getParam(GlobalConstant.VALID) == false) {
			utilSvc.addStatus(Response.ERROR, Response.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_VALIDATION_ERROR",prefCacheUtil.getLang(request)), response);
			return;
		}
		...
		if (!inputList.containsKey("LOGIN_FORM_USERNAME")) {
			utilSvc.addStatus(Response.ERROR, Response.EXECUTIONFAILED, prefCacheUtil.getPrefText("LOGIN_SERVICE", "LOGIN_SERVICE_BADUSERNAME",prefCacheUtil.getLang(request)), response);
		...
		if (user == null){
			...
			/*	if (SecurityUtils.containsPermission(user, "ADMAREA", "R") ) {
				...
				utilSvc.addStatus(Response.INFO, Response.SUCCESS, "Authenticated", response);
			} else if (!user.isEmailConfirm()){
				
				utilSvc.addStatus(Response.ERROR, Response.EXECUTIONFAILED, prefCacheUtil.getPrefText("LOGIN_SERVICE", "LOGIN_SERVICE_CHECK_EMAIL_CONFIRM",prefCacheUtil.getLang(request)), response);
			} else {
				
				utilSvc.addStatus(Response.ERROR, Response.EXECUTIONFAILED, prefCacheUtil.getPrefText("LOGIN_SERVICE", "LOGIN_SERVICE_BADPASSWORD",prefCacheUtil.getLang(request)), response);
			}
		}
	}
	/*
	...
	*/
	...
	public User findUser(String username){
		...
		User user = new User("admin","$2a$04$zsrGCqvAXRsa6qwTMAznjuC9V3OZKbyzRUg9NPVuiPMhRT/RJDWey","admin","admin","admin","33333","en");
